...
- **Phase progress:** 9 / 20 done; T1.10 IN PROGRESS (6/9 sub-tasks done)
- **Overall progress:** ~33% (Phase 1 watershed — 67% of T1.10 done)
- **Next milestone:** T1.10.9 final wire-up — new shell becomes primary
- **Active sub-task:** T1.10.7 — Extract bosses.js (boss state machine + archetypes + BOSSES_BY_CHAPTER expansion)
- **Critical for T1.10.9:** localStorage migration shim allow-list = 8 keys (T1.10.6 ephemeral, no change)
- **Cumulative core/ LoC:** ftue 475 + progression 1128 + grid 603 + heroes 3972 + channels 457 + stagger 1021 = **7656 LoC** in src/core/ so far
- **Last updated:** 2026-05-11 by CTO (after T1.10.6 review)
